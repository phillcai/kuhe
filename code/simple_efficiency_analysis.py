...
def create_simple_distribution_plot(df, efficiency_col, targets):
    """创建简化的分布图"""
    q75, q90, q95 = targets
    ...
    # 1. 效率分布直方图
    ax1 = axes[0, 0]
    ax1.hist(df[efficiency_col], bins=30, alpha=0.7, color='skyblue', edgecolor='black')
    ...
    # 2. 箱线图
    ax2 = axes[0, 1]
    ax2.boxplot(df[efficiency_col], vert=True)
    ...
    # 3. 累积分布图
    ax3 = axes[1, 0]
    sorted_efficiency = sorted(df[efficiency_col])
    cumulative_prob = [i/len(sorted_efficiency) for i in range(1, len(sorted_efficiency) + 1)]
    ...
    # 创建统计信息表格
    stats = df[efficiency_col].describe()
    table_data = [
        ['统计指标', '数值'],
        ['样本数量', f"{len(df):,}"],
        ['平均值', f"{stats['mean']:.4f}"],
...
def create_target_analysis(df, efficiency_col, targets):
    ...
    for i in range(len(bins) - 1):
        count = ((df[efficiency_col] >= bins[i]) & (df[efficiency_col] < bins[i+1])).sum()
        if i == len(bins) - 2:  # 最后一个区间包含最大值
            count = (df[efficiency_col] >= bins[i]).sum()
...
def main():
    ...
    # 生成分布图
    print("\n生成分布图...")
    fig1 = create_simple_distribution_plot(df_clean, efficiency_col, targets)
